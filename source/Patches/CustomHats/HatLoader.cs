    ...
    internal static class HatLoader
    {
        ...
        private static List<HatData> DiscoverHatBehaviours()
        {
            ...
            try
            {
                ...
                var twitchPath = TownOfUs.RuntimeLocation + "\\touthats.catalog";
                Addressables.AddResourceLocator(Addressables.LoadContentCatalog(twitchPath).WaitForCompletion());
                var all_twitch_previews_locations = Addressables.LoadResourceLocationsAsync("touthats").WaitForCompletion();
                var twitchAssets = Addressables.LoadAssetsAsync<HatData>(all_twitch_previews_locations, null, false).WaitForCompletion();
                var twitchArray = new Il2CppSystem.Collections.Generic.List<HatData>(twitchAssets.Pointer);
                hatBehaviours.AddRange(twitchArray.ToArray());
            }
            catch (Exception ex)
            {
                Log.LogError($"Error whilst loading modded hats");
